...
namespace EuropaEditor.Editors
{
    ...
    class MeshRenderer : ViewModelBase
    {
        ...
        private Vector3D _cameraDirection = new Vector3D(0.0, 0.0, 10.0);
        ...
        private Point3D _cameraPosition = new Point3D(0.0, 0.0, 10.0);
        ...
        private Point3D _cameraTarget = new Point3D(0.0, 0.0, 0.0);
        ...
        public Point3D OffsetCameraPosition => new Point3D(CameraPosition.X + CameraPosition.X, CameraPosition.Y + CameraTarget.Y, CameraPosition.Z + CameraTarget.Z);
            ...
            foreach (var mesh in lod.Meshes)
            {
                ...
                using (var reader = new BinaryReader(new MemoryStream(mesh.Vertices)))
                    for (int i = 0; i < mesh.VertexCount; ++i)
                    {
                        ...
                        var normalZ = Math.Sqrt(Math.Max(Math.Min(1f - (normalX * normalX + normalY * normalY), 1f), 0f)) * ((signs & 0x2) - 1f);
